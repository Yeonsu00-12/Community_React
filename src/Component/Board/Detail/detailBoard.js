    ...
    <>
      ...
      <D.Container>
        ...
        <D.Info>
          <D.Info_div>
            <D.AuthorImg
              src={`http://localhost:4000/${post?.authorProfile}`}
              alt="userProfile"
            />
          ...
          {isAuthor && (
            <div>
              <D.Edit onClick={editPostHandler} type="click">
                수정
              </D.Edit>
              <D.Edit onClick={toggleModal} type="submit">
                삭제
              </D.Edit>
            </div>
          )}
      ...
      <D.Content>
        <div>
          <D.ContentImg src={`http://localhost:4000/${post?.image}`} />
        </div>
        <D.Text>{post?.content}</D.Text>
      </D.Content>
      <D.ViewBox>
        <D.Views>
          <p style={{ margin: "0" }}>{post?.views}</p>
          <p style={{ margin: "0" }}>조회수</p>
        </D.Views>
        <D.Views>
          <p style={{ margin: "0" }}>{post?.commentCount}</p>
